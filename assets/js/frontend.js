/**
 * Frontend JavaScript for Bangladesh Tax Calculator
...
(function () {
    ...
    // Tax calculation utility functions - Exemption limits removed as per new requirement
    ...
    function calculateZeroTaxSlabLimit({ gender, above65, disabled, freedomFighter, taxYear }) {
        ...
        let baseLimit = 0,
            femaleLimit = 0,
            seniorCitizenLimit = 0,
            thirdGenderLimit = 0,
            disabledLimit = 0,
            freedomFighterLimit = 0;
        ...
        if (gender === 'female') {
            femaleLimit = baseLimit + 50000;
        }
        if (above65) {
            seniorCitizenLimit = baseLimit + 50000;
        }
        if (gender === 'third_gender') {
            thirdGenderLimit = baseLimit + 125000;
        }
        if (disabled) {
            disabledLimit = baseLimit + 125000;
        }
        if (freedomFighter) {
            freedomFighterLimit = baseLimit + 150000;
        }

        let finalBaseLimit = Math.max(baseLimit, femaleLimit, seniorCitizenLimit, thirdGenderLimit, disabledLimit, freedomFighterLimit);

        return finalBaseLimit;
    }
    ...
    function handleFormSubmit(event) {
        ...
        if (!form) {
            console.error('Form not found in event target');
            return;
        }

        const wrapper = form.closest('.bd-tax-calculator-wrapper');
        if (!wrapper) {
            console.error('Tax calculator wrapper not found');
            return;
        }

        const resultContainer = wrapper.querySelector('.bd-tax-result');
        if (!resultContainer) {
        ...
        const observer = new MutationObserver(function (mutations) {
            mutations.forEach(function (mutation) {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    // Check if any added nodes contain tax calculator forms
                    mutation.addedNodes.forEach(function (node) {
                        if (node.nodeType === 1) { // Element node
                            if (node.classList && node.classList.contains('bd-tax-calculator-wrapper')) {
                                initializeTaxCalculator();
                            } else if (node.querySelector && node.querySelector('.bd-tax-calculator-wrapper')) {
                                initializeTaxCalculator();
                            }
                        }
                    });
